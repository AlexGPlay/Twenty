...
export async function register(fields: RegisterFields, req: Request): Promise<UserResponse> {
  ...
  const userYears =
    new Date(new Date().getTime() - userFields.birthday.getTime()).getFullYear() - 1970;
  if (userYears < 14)
    return { errors: [{ field: "birthday", message: "Debes ser mayor de 14 años" }] };
